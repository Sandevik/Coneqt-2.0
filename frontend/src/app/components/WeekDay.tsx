...
export default function WeekDay({children, focusDay, currentWeek, currentYear, setFocusDay, handleOnDrag, handleDrop, handleDragOver, selectedShift, setSelectedShift, shifts, day}: {day: number, children?: React.ReactNode, shifts: Record<number, _Shift[]>, setSelectedShift: React.Dispatch<React.SetStateAction<_Shift|null>>, selectedShift: _Shift|null, handleDragOver: (e: React.DragEvent) => void, handleDrop: (e: React.DragEvent, toWeekDay: number) => void, handleOnDrag: (e: React.DragEvent, shift: _Shift) => void, setFocusDay: React.Dispatch<React.SetStateAction<number|null>>, focusDay: number | null, currentWeek: number, currentYear: number}) {
  return (
    <li className={`bg-gray-100 rounded-md h-[calc(100dvh-104px)] transition-all  ${focusDay === null ? "" : focusDay === day ? "w-[99dvw] z-30 " : "w-auto hidden"}`} onDrop={(e) => handleDrop(e, day)} onDragOver={handleDragOver}>
      <div className={`justify-between p-1 px-2 text-lg ${focusDay === day ? "hidden" : "flex"}`}>
        ...
        <div className="font-semibold">{getDateRangeOfWeek(currentWeek, currentYear)[day - 1]}</div>
      </div>
      {focusDay === null && <div className='flex gap-2 flex-col p-2'>
        {shifts[day].map(shift => <Shift handleOnDrag={handleOnDrag} draggable key={shift.uuid} selectedShift={selectedShift} setSelectedShift={setSelectedShift} shift={shift}/>)}
      </div>}
